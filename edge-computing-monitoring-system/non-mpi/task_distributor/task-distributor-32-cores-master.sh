...
# Build HOSTS_ARRAY based on NUM_NODES
PHYSICAL_NODES=([1]=rpi3-01 [2]=rpi3-02 [3]=rpi3-03 [4]=rpi3-04 [5]=rpi3-05 [6]=rpi3-06 [7]=rpi3-07 [8]=rpi3-08)
declare -a HOSTS_ARRAY
if [ "$NUM_NODES" -le 8 ]; then
  for ((node=1; node<=NUM_NODES; node++)); do
    HOSTS_ARRAY[$node]=${PHYSICAL_NODES[$node]}
  done
else
